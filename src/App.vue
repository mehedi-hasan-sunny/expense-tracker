<template>
    <v-app :class="{'white-mode': !settingsData.darkMode}" id="app">
        <v-toolbar :color="settingsData.darkMode ? '#212121' : '#ffffff'" :dark="settingsData.darkMode" app
                   clipped-right fixed>
            <v-spacer></v-spacer>
        ...
        <v-footer :color="settingsData.darkMode ? '#212121' : '#ffffff'" :dark="settingsData.darkMode" app
                  class="border-top"
                  id="btm-hider">
            <v-layout class="ma-0" row>
                <v-flex :class="{'active-tab': link.route===$route.path}" :key="link.icon"
                        @click.prevent="title = link.title, goToRoute(link.route)"
